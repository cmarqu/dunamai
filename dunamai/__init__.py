...
class Version:
    ...
    def from_pijul(
        ...
        dirty = msg.strip() != ""
        if dirty:
            raise RuntimeError("Cannot check Pijul projects while the working tree is dirty")

        code, msg = _run_cmd("pijul log --limit 1 --output-format json")
        if not msg.strip():
            return cls("0.0.0", dirty=dirty)
